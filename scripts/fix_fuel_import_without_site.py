...
def patch(old: str, new: str) -> None:
    ...
    if new in s:
        return
    if old not in s:
        raise SystemExit(f"fuel receipt patch anchor not found: {old[:100]}")
    s = s.replace(old, new, 1)


# The no-site import and vehicle-to-site mapping are already committed in
# FuelManagement.tsx. Keep this prebuild hook idempotent and use it only for
# the receipt feature until the generated source is folded back into the file.
patch(

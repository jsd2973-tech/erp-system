...
rep('import { FileSpreadsheet, Fuel, Plus, RefreshCcw, Search, Trash2, Upload } from "lucide-react";','import { Download, FileSpreadsheet, Fuel, Plus, RefreshCcw, Search, Trash2, Upload } from "lucide-react";','icon')
rep('  const [records, setRecords] = useState<FuelRecord[]>([]);\n  const [loading, setLoading] = useState(true);','  const [records, setRecords] = useState<FuelRecord[]>([]);\n  const [referenceRecords, setReferenceRecords] = useState<FuelRecord[]>([]);\n  const [loading, setLoading] = useState(true);','state')
rep('  useEffect(() => { void load(); }, [month]);\n\n  const sites = useMemo','''  useEffect(() => { void load(); }, [month]);
  useEffect(() => {
  ...
  const sites = useMemo''','loader')
rep('  const siteSummary = useMemo(() => summarize("site_name"), [filtered]);\n\n  const onFile = async','''  const siteSummary = useMemo(() => summarize("site_name"), [filtered]);
  ...
  const onFile = async''','exports')
rep('        <button type="button" onClick={() => fileInput.current?.click()}><Upload size={16} /> 명세서 가져오기</button>\n        <button type="button" onClick={() => setManualOpen((value) => !value)}><Plus size={16} /> 직접 입력</button>','''        <button type="button" onClick={() => fileInput.current?.click()}><Upload size={16} /> 명세서 가져오기</button>
        ...
        <button type="button" onClick={() => setManualOpen((value) => !value)}><Plus size={16} /> 직접 입력</button>''','buttons')
rep('''        <label><span>현장</span><input value={manual.site_name} onChange={(event) => setManual({ ...manual, site_name: event.target.value })} placeholder="공장" /></label>
        ...
        <label><span>차량/장비번호 *</span><input value={manual.vehicle_number} onChange={(event) => setManual({ ...manual, vehicle_number: event.target.value })} placeholder="세종03가1166" /></label>''','''        <label><span>현장</span><input list="fuel-site-options" value={manual.site_name} onChange={(event) => setManual({ ...manual, site_name: event.target.value })} placeholder="공장" /><datalist id="fuel-site-options">{allSites.map((name) => <option key={name} value={name} />)}</datalist></label>
        ...
        <label className="fuel-vehicle-entry"><span>차량/장비번호 *</span><input list="fuel-vehicle-options" value={manual.vehicle_number} onChange={(event) => { const value=event.target.value; setManual({ ...manual, vehicle_number:value }); if (vehicleOptions.includes(value)) applyVehicleProfile(value); }} onBlur={() => { if (vehicleOptions.includes(manual.vehicle_number)) applyVehicleProfile(manual.vehicle_number); }} placeholder="번호 입력 또는 선택" /><datalist id="fuel-vehicle-options">{vehicleOptions.map((name) => <option key={name} value={name} />)}</datalist></label>''','entry')
rep('''      </div>
      <div className="fuel-manual-total"><span>예상 합계</span>''','''      </div>
      ...
      <div className="fuel-manual-total"><span>예상 합계</span>''','quick')
p.write_text(s)
css=Path('src/features/fuel/fuelManagement.css')
css.write_text(css.read_text()+'''\n.fuel-quick-vehicles{margin:12px 0 4px;padding:12px;border:1px solid #dbe5f1;border-radius:12px;background:#f8fbff}.fuel-quick-vehicles>span{display:block;font-weight:700;margin-bottom:8px}.fuel-quick-vehicles>div{display:flex;flex-wrap:wrap;gap:6px}.fuel-quick-vehicles button{border:1px solid #cbd8e8;background:#fff;border-radius:999px;padding:6px 10px;cursor:pointer}.fuel-quick-vehicles button:hover{border-color:#2563eb;color:#1d4ed8}.fuel-quick-vehicles small{display:block;margin-top:8px;color:#64748b}.fuel-head-actions{flex-wrap:wrap}\n''')
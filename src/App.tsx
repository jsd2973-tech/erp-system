...
export default function App() {
  ...
  const itemOptions = useMemo(
    () => items.map((i) => ({ label: `${i.code} / ${i.name}`, value: i.name, code: i.code, name: i.name })).filter((i) => i.name),
    [items]
  );
          ...
          <section className="card">
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>{rows.map((r, i) => <tr key={r.id}><td><SearchSelect value={r.item} options={itemOptions} onChange={(v) => updateRow(i, "item", v)} placeholder="품목명이나 코드 입력" /></td><td><input value={r.spec} readOnly /></td><td><input className="right" value={r.qty} onChange={(e) => updateRow(i, "qty", e.target.value)} /></td><td><input className="right" value={r.price} onChange={(e) => updateRow(i, "price", e.target.value)} /></td><td><input className="right" value={r.supply} onChange={(e) => updateRow(i, "supply", e.target.value)} /></td><td><input className="right" value={r.vat} onChange={(e) => updateRow(i, "vat", e.target.value)} /></td><td className="right bold">{money(r.total)}</td><td><button className="icon" onClick={() => setRows(rows.length === 1 ? [emptyRow()] : rows.filter((_, idx) => idx !== i))}><Trash2 size={16} /></button></td></tr>)}</tbody>
              </table>
            </div>

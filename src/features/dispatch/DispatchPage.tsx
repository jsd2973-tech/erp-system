...
import type { DispatchDriver, DispatchOrder, DispatchOrderForm, DispatchOrderVehicle, DispatchOrderWithVehicles, DispatchReferenceOption, DispatchVehicle, DispatchView } from "./dispatchTypes";
...
export default function DispatchPage({ view, supabase, vendors, items, isAdmin, onNavigate, onNotify }: DispatchPageProps) {
  ...
  const loadDispatchData = useCallback(async () => {
    setLoading(true);
    setError("");
    const [vehicleResult, driverResult, orderResult, assignmentResult] = await Promise.all([
    ...
    const firstError = vehicleResult.error || driverResult.error || orderResult.error || assignmentResult.error;
    if (firstError) {
  ...
  const saveOrder = async (form: DispatchOrderForm) => {
    const vendor = vendors.find((option) => option.id === form.vendor_id);
    const item = items.find((option) => option.id === form.item_id);
    if (!vendor || !item) {
      setError("선택한 거래처 또는 품목을 찾을 수 없습니다. 기초자료를 새로고침해 주세요.");
      return false;
    }
  ...
  return (
    <div className="dispatch-page">
      ...
      {loading ? <div className="dispatch-loading">배차관리 자료를 불러오는 중...</div> : <>
        {view === "dispatch_register" && <><DispatchRegister vendors={vendors} items={items} vehicles={vehicles} editingOrder={editingOrder} saving={saving} onSave={saveOrder} onCancelEdit={() => setEditingOrder(null)} /><DispatchList orders={orders} vehicles={vehicles} onEdit={editOrder} compact /></>}
